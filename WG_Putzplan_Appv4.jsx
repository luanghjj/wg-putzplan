    ...
    <div style={{fontFamily:F,minHeight:"100vh",background:"#F8FAFC",maxWidth:520,margin:"0 auto",position:"relative"}}>
      ...
      {scr==="login"?<LoginScr t={t} st={st} sv={sv} onLogin={u=>{setUser(u);setScr("plan")}}/>:
        <div style={{padding:"14px 14px 32px",maxWidth:520,margin:"0 auto"}}>
          <NavBar t={t} scr={scr} set={setScr} user={user} hp={hp} onLogout={()=>{setUser(null);setScr("login")}}/>
          {scr==="plan"&&<PlanScr t={t} st={st} user={user} hp={hp} doDone={doDone} doUndo={doUndo} isC={isC} ph={ph} vp={setPhView} openTut={setTutView}/>}
...
function NavBar({t,scr,set,user,hp,onLogout}){
  ...
  const items=[{id:"plan",icon:"📋",l:t.plan,s:true},{id:"leaderboard",icon:"🏆",l:t.leaderboard,s:true},{id:"rules",icon:"📜",l:t.rules,s:true},{id:"history",icon:"📊",l:t.history,s:hp("view_history")},{id:"admin",icon:"⚙️",l:t.admin,s:showA}];
  const ni={flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:4,padding:"7px 4px",border:"none",borderRadius:10,background:"transparent",fontSize:11,fontWeight:500,color:"#64748B",cursor:"pointer",fontFamily:F};
  ...
  return <div style={{background:"#fff",borderRadius:16,padding:12,marginBottom:14,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,paddingBottom:10,marginBottom:10,borderBottom:"1px solid #F1F5F9"}}>
      ...
      <div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:700,color:"#1E293B"}}>{user?.name} <span style={{fontSize:12}}>{RI[user?.role]}</span></div><div style={{fontSize:11,color:"#94A3B8"}}>{user?.room!=="—"?user?.room:t[user?.role]}</div></div>
      <button style={{marginLeft:"auto",background:"none",border:"1px solid #E2E8F0",borderRadius:8,padding:"4px 12px",fontSize:12,color:"#64748B",cursor:"pointer",fontFamily:F}} onClick={onLogout}>{t.logout}</button>
    </div>
    <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{items.filter(i=>i.s).map(i=><button key={i.id} style={{...ni,...(scr===i.id?nia:{})}} onClick={()=>set(i.id)}><span style={{fontSize:13}}>{i.icon}</span><span>{i.l}</span></button>)}</div>
  </div>;
}
...
function PlanScr({t,st,user,hp,doDone,doUndo,isC,ph,vp,openTut}){
  ...
  const TR=({task,areaId,area})=>{
    ...
    return <div style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px",borderRadius:10,marginBottom:4,background:comp?(area?.bg||"#F0FFF4"):"transparent"}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        {comp&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:4,marginLeft:32}}>
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#fff",borderRadius:16,padding:14,marginBottom:14,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
      <div><h2 style={{margin:0,fontSize:20,color:"#1E293B",fontFamily:F}}>{t.plan}</h2><p style={{margin:"4px 0 0",fontSize:13,color:"#64748B"}}>{t.kwNum} {wk} · {day}</p></div>
...
function LeaderScr({t,st}){
  ...
  const cPen=u=>{const rot=grot(wk,st.rooms,st.weeklyAreas);let m=0;st.weeklyAreas.forEach(a=>{if(rot[a.id]===u.roomId)a.tasks.forEach(ta=>{if(!st.completions.find(c=>c.taskKey===ta.de&&c.areaId===a.id&&c.week===wk))m++;});});return m*(st.penaltyRate||5);};
  return <div>
    ...
    <div style={{background:"#fff",borderRadius:12,padding:14,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      ...
      {users.map(u=>{const p=cPen(u);return <div key={u.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:"1px solid #F1F5F9"}}><span style={{flex:1,fontSize:14,fontWeight:500}}>{u.name}</span>{p>0?<span style={{fontSize:14,fontWeight:700,color:"#EF4444"}}>{t.owes}: {p.toFixed(2)}€</span>:<span style={{fontSize:13,color:"#10B981",fontWeight:600}}>✓ OK</span>}</div>;})}
    </div>
  </div>;
}
...
function TaskMgr({t,st,sv,show}){
  ...
  // Tutorial edit modal
  const TutEdit=()=> <div style={ov} onClick={()=>setEditTut(null)}><div style={{...mod,maxWidth:460,padding:0,maxHeight:"90vh",overflow:"auto"}} onClick={e=>e.stopPropagation()}>
    ...
    <div style={{padding:"12px 20px 20px"}}>
      {tutSteps.map((s,i)=> <div key={i} style={{padding:10,background:"#F8FAFC",borderRadius:10,marginBottom:8,border:"1px solid #E2E8F0"}}>
...
function BonusCfg({t,st,sv,rpin,show}){
  ...
  const go=async()=>{const ok=await rpin();if(!ok)return;sv({...st,penaltyRate:Number(rate),rewardText:rew});show("✓");};
  return <div style={{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
    ...
    <button style={btnP} onClick={go}>{t.save}</button>
  </div>;
}
