...
export default function LeaderScreen({t,st,user}){
  ...
  return <div>
    {w&&w.month>0&&<div style={{position:"relative",overflow:"hidden",background:"linear-gradient(135deg,#FFF8E1,#FFE082)",borderRadius:18,padding:22,marginBottom:16,textAlign:"center",boxShadow:"0 8px 28px rgba(255,204,0,0.22)",border:"1px solid rgba(255,204,0,0.2)"}}><div style={{position:"absolute",top:0,left:0,width:"60%",height:"100%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.55),transparent)",animation:"shine 3.2s ease-in-out infinite",pointerEvents:"none"}}/><div style={{fontSize:44,animation:"floaty 2.6s ease-in-out infinite"}}>🏆</div><h3 style={{margin:"8px 0 4px",fontFamily:F,color:"#7B6B00",fontSize:18,fontWeight:700,letterSpacing:"-0.022em"}}>{t.monthlyWinner}</h3><div style={{fontSize:24,fontWeight:800,color:"#5D4E00",fontFamily:F}}>{w.name}</div><div style={{fontSize:14,color:"#8B7500"}}>{w.month} {t.points} · {st.rewardText||t.defaultReward}</div></div>}
    ...
    {ranked.map((u,i)=> <div key={u.id} style={{position:"relative",overflow:"hidden",display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:i===0?"linear-gradient(135deg,rgba(255,248,225,0.95),rgba(255,236,179,0.85))":"rgba(255,255,255,0.72)",backdropFilter:"blur(20px)",borderRadius:14,marginBottom:6,boxShadow:i===0?"0 6px 22px rgba(255,193,7,0.28)":C.shadowSm,border:i===0?"1px solid rgba(255,193,7,0.45)":`1px solid ${C.border}`,animation:"fadeUp 0.25s ease"}}>
      {i===0&&u.month>0&&<div style={{position:"absolute",top:0,left:0,width:"50%",height:"100%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.5),transparent)",animation:"shine 3.6s ease-in-out infinite",pointerEvents:"none"}}/>}
